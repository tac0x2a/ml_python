...
#### カーネル
線形SVM(`LinearSV{C,R}`)だと，入力特徴量の相互作用(積)とか多項式項を加えて特徴量を追加することで，超平面での複雑なモデルを構築していた．非線形な特徴量データに加えることで，非線形モデルを作ることができる．
...
+ `gamma`: 小さいほどカーネルの直径が大きくなる(多くの点を近いと判定する)ためモデルが単純になる．
+ `kernel`: カーネル．`rbf`
...
+ Bad: サンプル個数が多くなると実行時間やメモリの面で辛くなってくる．データの前処理とパラメータ調整を注意深く行う必要がある．

勾配ブースティング等と比べると，上記データ前処理&パラメータ調整の面と，結果の説明の面で辛い．
SVMは予測結果について(決定木等と比べて)理由を説明するのが困難．
